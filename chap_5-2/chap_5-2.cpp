...
/* 3번
알파가 너무 큰 경우에는 평균 밝기를 기준으로 픽셀들이 양극단으로 강하게 밀려나면서 명암비가 극도로 높아지는데, 이로 인해 밝은 영역은 순백색(255)으로,
어두운 영역은 순검정색(0)으로 포화되는 클리핑 현상이 발생하여 영상의 세부적인 질감이나 정보가 손실되고 이미지가 매우 거칠게 표현된다.
또한 알파가 너무 작은 경우(0에 가깝거나 음수인 경우)에는 픽셀 간의 밝기 차이가 줄어들면서 모든 픽셀값이 영상의 평균값 근처로 수렴하게 되는데,
이로 인해 밝고 어두운 경계가 모호해지는 저대비 현상이 나타나며 알파가 -1에 가까워질수록 영상은 아무런 형체를 알아볼 수 없는 평탄한 회색조 이미지로 변하게 된다.
